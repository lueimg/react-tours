...
const rootUrl = `${baseUrl}/coupon_used_modes`;
...
export const loadCouponModeList = () => {
    return Observable
...
export const loadCouponModeForSelect = () => {
    return Observable
        .fromPromise(FirebaseApi.db().ref(rootUrl).once('value'))
        .map((snapshot) => snapshot.val())
        .map(formartObjectList)
...
export const loadCouponModeById = (id) => {
    return Observable
