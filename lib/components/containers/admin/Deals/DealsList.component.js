...
import { loadDealsList } from 'components/actions';
...
class DealsList extends React.Component {
    ...
    componentDidMount = () => {
        this.props.actions.loadDealsList()
            .subscribe((list) => {
    ...
    add = () => {
        this.props.history.push({ pathname: '/admin/deals/add' });
    }

    render() {
        return (
            <PageBase title="Deals " navigation="Deals / List" >
...
const RowItem = (props) => {
    return (
        <TableRow key={props.id}>
            ...
            <TableRowColumn>{props.deal_type_id}</TableRowColumn>
            <TableRowColumn>{props.start_date}</TableRowColumn>
            <TableRowColumn>{props.end_date}</TableRowColumn>
            <TableRowColumn style={styles.columns.edit}>
                <Link className="button" to={`/admin/deals/${props.id}/edit`}>
...
export default withRouter(connect(null, mapDispatchToProps)(DealsList));
